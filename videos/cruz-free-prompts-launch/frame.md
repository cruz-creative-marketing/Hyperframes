...
description: >
  ...
  Montserrat in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
colors:
  ink-black: "#0A0909"
  ink-black-alt: "#141212"
  fire-orange: "#A5F0B1"
  cream: "#FFFFFF"
  cream-muted: "#CCCCCC"
  cream-hint: "#8C8888"
  border-dark: "#2E2929"
  ink-on-orange-muted: "rgba(10,9,9,0.75)"
  ink-on-orange-hint: "rgba(10,9,9,0.55)"
  ink-on-orange-faint: "rgba(10,9,9,0.40)"
  ink-on-orange-border: "rgba(10,9,9,0.20)"

typography:
  # — reading ramp —
  body:    { fontFamily: "Montserrat", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Montserrat", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Montserrat", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Montserrat, lowercase, negative tracking) —
  h3:      { fontFamily: "Montserrat", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Montserrat", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Montserrat", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Montserrat", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Montserrat", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Montserrat", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Montserrat", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Montserrat", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Montserrat", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Palette hand-corrected (orchestrator):** the automatic remix inverted the registers. Cruz Creative's
  site is dark-mode: `ink-black` #0A0909 is the ground, `cream` #FFFFFF is text, `cream-muted` #CCCCCC is
  secondary text, `fire-orange` = brand mint #A5F0B1 (the only accent). "Orange register" = mint ground with
  #0A0909 ink. Surfaces #141212 / #1E1A1A may back cards. Brand danger tone #F0A5A5 is reserved for the
  single "generic AI" negative label in the compare frame — nowhere else.
- **Mono** — chrome stays IBM Plex Mono; the captured site's prompt/code text is Courier-style mono, so a
  code block may use IBM Plex Mono too.
...
- **Fonts** — already set to **Montserrat** (display) / **Courier New** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** Montserrat `display` at 13cqw puts a single lowercase word
...
**Montserrat** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Montserrat 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Montserrat body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Montserrat `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Montserrat display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Montserrat `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Montserrat lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Montserrat `display`/`h1`
...
row of three top-border-only stat-cards — big Montserrat-900 numeral in `{colors.fire-orange}`, Montserrat
...
**Ground** ink-black. **Composes** fadelist (3 stacked Montserrat-900 words at 1.0/0.5/0.22), fadelist-title.
...
Montserrat `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Montserrat display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Montserrat display; never add a second accent color.
...
- No serif companion; chrome is never Montserrat.
...
- **Type** — Montserrat lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Montserrat + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
